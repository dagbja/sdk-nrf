...
#define LWM2M_DEVICE_STRING_MAX_LEN 200
...
int lwm2m_device_avail_power_sources_set(lwm2m_device_power_source_t * power_sources, uint8_t power_source_count)
{
    ...
    {
        if (((int)power_sources[i] < 0) || ((int)power_sources[i] > 7) || ((int)power_sources[i] == 3))
        {
            return EINVAL;
        }
    }
    ...
    for (int i = 0; i < power_source_count; i++)
    {
        device_obj_instance->avail_power_sources.val.p_uint8[i] = (uint8_t)power_sources[i];
    ...
    device_obj_instance->battery_status = (int32_t)DEVICE_BATTERY_STATUS_UNKNOWN;
...
int lwm2m_device_power_source_voltage_set(lwm2m_device_power_source_t power_source, int32_t value)
{
    ...
    if (((int)power_source < 0) || ((int)power_source > 7) || ((int)power_source == 3))
    {
        return EINVAL;
    }
    ...
    {
        if (device_obj_instance->avail_power_sources.val.p_uint8[i] == (uint8_t)power_source)
        {
...
int lwm2m_device_battery_level_set(uint8_t battery_level)
{
    ...
    for (int i = 0; i < (device_obj_instance->avail_power_sources.len); i++)
    {
        if (device_obj_instance->avail_power_sources.val.p_uint8[i] == (uint8_t)DEVICE_POWER_SOURCE_INTERNAL_BATTERY)
        {
...
int lwm2m_device_battery_status_set(lwm2m_device_battery_status_t battery_status)
{
    ...
    if (((int)battery_status < 0) || ((int)battery_status > 6))
    {
        return EINVAL;
    }
    ...
    for (int i = 0; i < (device_obj_instance->avail_power_sources.len); i++)
    {
        if (device_obj_instance->avail_power_sources.val.p_uint8[i] == (uint8_t)DEVICE_POWER_SOURCE_INTERNAL_BATTERY)
        {
            device_obj_instance->battery_status = (int32_t)battery_status;
            lwm2m_device_notify_resource(LWM2M_DEVICE_BATTERY_STATUS);
            return 0;
        }
    }
    device_obj_instance->battery_status = DEVICE_BATTERY_STATUS_NOT_INSTALLED;
...
int lwm2m_device_type_set(char * device_type)
{
    ...
    if (strlen(device_type) > LWM2M_DEVICE_STRING_MAX_LEN)
    {
        return E2BIG;
    }
...
int lwm2m_device_software_version_set(char * software_version)
{
    ...
    if (strlen(software_version) > LWM2M_DEVICE_STRING_MAX_LEN)
    {
        return E2BIG;
    }
...
int lwm2m_device_error_code_add(lwm2m_device_error_code_t error)
{
    ...
    if (((int)error < 0) || ((int)error > 8))
    {
        return EINVAL;
    }

    if (error == DEVICE_ERROR_CODE_NO_ERROR)
    {
        device_obj_instance->error_code.len = 1;
        device_obj_instance->error_code.val.p_int32[0] = (int32_t)error;
        lwm2m_device_notify_resource(LWM2M_DEVICE_ERROR_CODE);
        return 0;
    }

    if ((len == 1) && (device_obj_instance->error_code.val.p_int32[0] == (int32_t)DEVICE_ERROR_CODE_NO_ERROR))
    {
        device_obj_instance->error_code.val.p_int32[0] = (int32_t)error;
        lwm2m_device_notify_resource(LWM2M_DEVICE_ERROR_CODE);
        return 0;
    ...
    for (int i = 0; i < len; i++)
    {
        if (device_obj_instance->error_code.val.p_int32[i] == (int32_t)error)
        {
            return 0;
        }
    }
    ...
    device_obj_instance->error_code.val.p_int32[len] = (int32_t)error;
    lwm2m_device_notify_resource(LWM2M_DEVICE_ERROR_CODE);
...
int lwm2m_device_error_code_remove(lwm2m_device_error_code_t error)
{
    ...
    for (int i = 0; i < len; i++)
    {
        if (device_obj_instance->error_code.val.p_int32[i] == (int32_t)error)
        {
            if (len == 1)
            {
                device_obj_instance->error_code.val.p_int32[0] = (int32_t)DEVICE_ERROR_CODE_NO_ERROR;
                lwm2m_device_notify_resource(LWM2M_DEVICE_ERROR_CODE);
                return 0;
            }
            else
...
int lwm2m_device_memory_total_set(int32_t memory_total)
{
    ...
    device_obj_instance->memory_total = memory_total;
    lwm2m_device_notify_resource(LWM2M_DEVICE_MEMORY_TOTAL);

    return 0;
}

int __WEAK lwm2m_device_memory_free_read(void)
{
    return 0;
}
