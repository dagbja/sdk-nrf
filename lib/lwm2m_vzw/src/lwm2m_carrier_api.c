...
int lwm2m_carrier_avail_power_sources_set(lwm2m_carrier_power_source_t * power_sources, uint8_t power_source_count)
{
    ...
    {
        if ((power_sources[i] < LWM2M_CARRIER_POWER_SOURCE_DC) || (power_sources[i] > LWM2M_CARRIER_POWER_SOURCE_SOLAR) || ((int)power_sources[i] == 3))
        {
            return -EINVAL;
        }
    }
    ...
    device_obj_instance->battery_status = (int32_t)LWM2M_CARRIER_BATTERY_STATUS_UNKNOWN;
...
int lwm2m_carrier_power_source_voltage_set(lwm2m_carrier_power_source_t power_source, int32_t value)
{
    ...
    if ((power_source < LWM2M_CARRIER_POWER_SOURCE_DC) || (power_source > LWM2M_CARRIER_POWER_SOURCE_SOLAR) || ((int)power_source == 3))
    {
        return -EINVAL;
    }
...
int lwm2m_carrier_error_code_add(lwm2m_carrier_error_code_t error)
{
    ...
    if ((len == 1) && (device_obj_instance->error_code.val.p_int32[0] == (int32_t)LWM2M_CARRIER_ERROR_CODE_NO_ERROR))
    {
        device_obj_instance->error_code.val.p_int32[0] = (int32_t)error;
        lwm2m_device_notify_resource(LWM2M_DEVICE_ERROR_CODE);
        return 0;
...
int lwm2m_carrier_error_code_remove(lwm2m_carrier_error_code_t error)
{
    ...
    for (int i = 0; i < len; i++)
    {
        if (device_obj_instance->error_code.val.p_int32[i] == (int32_t)error)
        {
            if (len == 1)
            {
                device_obj_instance->error_code.val.p_int32[0] = (int32_t)LWM2M_CARRIER_ERROR_CODE_NO_ERROR;
                lwm2m_device_notify_resource(LWM2M_DEVICE_ERROR_CODE);
                return 0;
            }
            else
